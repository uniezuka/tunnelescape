...
* **Built Level 1** ("First Steps"): a Start Room (one portal) and an Exit Room, connected by a single red portal. Authored as `level_01.tres` rather than hardcoded scene references.
* **Tutorial "force click" pattern:** a `Tappable` base class (shared by `Portal` and the new `ExitDoor`) supports a pulsing highlight plus a level-controller-side input lock that ignores taps anywhere except the highlighted target. Both the Start Room's portal and the Exit Room's door use this on first appearance.
...
* The Exit Door is now its own tappable node (`ExitDoor`, extends `Tappable`) placed inside the exit room. Reaching the room via portal no longer auto-completes the level; the player must tap the door.
