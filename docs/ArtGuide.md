# Art Guide

## Visual Style

## Color Palette

## Room Themes

Each level (stage) is built around exactly one theme from the list below (see [GDD.md](GDD.md#10-art-direction)). Rooms inside that stage do **not** get labeled with the theme name itself — each room gets its own specific name drawn from that theme's list, e.g. Stage 5 (Library theme) has rooms named "The Librarian's Office" and "The Forgotten Archive," not a room simply called "Library."

Every stage has exactly one Start Room and one Exit Room (see GDD.md's [Rooms](GDD.md#rooms) and [Exit Door](GDD.md#exit-door) rules). Each theme has a fixed Start Room name and a fixed Exit Room name — every stage that uses a given theme reuses that same pair, so "The Library Entrance" always means the start of a Library stage, wherever it appears. The rest of a stage's rooms are picked freely from that theme's "Other Rooms" list.

The Exit Room name listed below is the plain flavor name — in-game it's always shown with an "(Exit)" suffix (e.g. "The Grand Reading Hall (Exit)"), so the player can recognize it immediately via Compass, Warp Scroll, or the Map overlay. The Start Room name is never suffixed; there's no ambiguity about where the player begins.

A stage only needs a handful of rooms per level. The same theme can be reused across multiple stages later in the game, drawing on the "Other Rooms" names that stage didn't already use.

### 📚 Library

* **Start Room:** The Library Entrance
* **Exit Room:** The Grand Reading Hall

Other rooms:

* The Librarian's Office
* The Children's Reading Room
* The Forgotten Archive
* The Botanical Conservatory
* The Potion Workshop
* The Astronomer's Study
* The Crystal Repository
* The Ancient Records Vault
* The Scriptorium
* The Card Catalog Hall
* The Rare Books Wing
* The Reading Nook

### 🗿 Temple

* **Start Room:** The Temple Entrance
* **Exit Room:** The Inner Sanctum

Other rooms:

* The Antechamber of Offerings
* The Hall of Idols
* The High Priest's Chamber
* The Sunken Sanctuary
* The Chamber of Echoes
* The Ritual Basin
* The Hall of Ancestors
* The Ossuary
* The Bell Tower
* The Cloister Garden
* The Chamber of Trials

### 🌿 Garden

* **Start Room:** The Garden Entrance
* **Exit Room:** The Hidden Grove

Other rooms:

* The Greenhouse
* The Topiary Maze
* The Rose Pavilion
* The Koi Pond
* The Gardener's Shed
* The Orchard
* The Sunken Grotto
* The Herb Nursery
* The Vine-Covered Trellis
* The Fountain Courtyard
* The Beehive Apiary
* The Wildflower Meadow

### 🧪 Laboratory

* **Start Room:** The Laboratory Entrance
* **Exit Room:** The Chief Scientist's Office

Other rooms:

* The Alchemy Bench
* The Specimen Room
* The Distillation Chamber
* The Research Archive
* The Observation Deck
* The Reagent Storeroom
* The Experiment Vault
* The Botanical Sample Room
* The Cold Storage Chamber
* The Testing Bay
* The Sterilization Room

### 💎 Crystal Chamber

* **Start Room:** The Mine Entrance
* **Exit Room:** The Geode Hollow

Other rooms:

* The Prism Hall
* The Mining Shaft
* The Resonance Chamber
* The Gem Vault
* The Luminous Cavern
* The Crystal Garden
* The Quarry Pit
* The Refraction Room
* The Miner's Rest
* The Shard Collection Hall
* The Glowstone Grotto

### ⚙️ Machinery Room

* **Start Room:** The Factory Entrance
* **Exit Room:** The Turbine Hall

Other rooms:

* The Gear Works
* The Boiler Room
* The Control Room
* The Conveyor Bay
* The Furnace Chamber
* The Pressure Valve Room
* The Assembly Line
* The Engineer's Workshop
* The Steam Vent Chamber
* The Spare Parts Depot
* The Clockwork Vault

### 🕯️ Shrine

* **Start Room:** The Shrine Entrance
* **Exit Room:** The Sacred Grove

Other rooms:

* The Prayer Alcove
* The Candlelit Vestibule
* The Offering Table
* The Meditation Room
* The Reliquary
* The Incense Chamber
* The Monk's Quarters
* The Silent Hall
* The Ancestor Wall
* The Blessing Basin
* The Chapel of Whispers

### 📦 Storage Room

* **Start Room:** The Warehouse Entrance
* **Exit Room:** The Vault of Odds and Ends

Other rooms:

* The Supply Depot
* The Crate Yard
* The Dusty Cellar
* The Loading Dock
* The Pantry
* The Archive Boxes Room
* The Forgotten Closet
* The Inventory Office
* The Cold Store
* The Warehouse Floor
* The Lost and Found

## Asset Specs
