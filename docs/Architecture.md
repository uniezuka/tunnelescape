...
│   └── tunnel/         Tunnel.tscn — reusable tappable tunnel node
├── scripts/
│   ├── autoload/       GameState.gd, SaveManager.gd, LevelLoader.gd
│   ├── systems/        movement, tunnel/teleport, move counter
...
* Each room is its own scene (`scenes/rooms/room_XX.tscn`), containing its tunnels, exit (if any), decorative theme, and optional power-up pickup.
* `Tunnel.tscn` is a reusable scene/prefab instanced inside each room, configured per-instance with its color and destination room id.
...
* Signals: `snake_case`, past-tense for events (`tunnel_entered`, `level_completed`).

## Signals & Communication Patterns

Prefer signals over direct references between systems (e.g. a `Tunnel` node emits `tunnel_entered(destination_room_id)` rather than calling into `LevelLoader` directly), so rooms/tunnels stay decoupled from the level-flow controller and can be reused across all 20 levels without per-level wiring.
